...
public class SlimeKelpFeature extends Feature<NoneFeatureConfiguration> {
    public SlimeKelpFeature(Codec<NoneFeatureConfiguration> p_66219_) {
        super(p_66219_);
    }
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> p_159956_) {
        ...
        if (worldgenlevel.getBlockState(blockpos1).is(Blocks.WATER)) {
            BlockState blockstate = ModItems.SlimeKelp.get().defaultBlockState();
            BlockState blockstate1 = ModItems.SlimeKelpPlant.get().defaultBlockState();
            ...
            for(int l = 0; l <= k; ++l) {
                if (worldgenlevel.getBlockState(blockpos1).is(Blocks.WATER) && worldgenlevel.getBlockState(blockpos1.above()).is(Blocks.WATER) && blockstate1.canSurvive(worldgenlevel, blockpos1)) {
                    ...
                    if (blockstate.canSurvive(worldgenlevel, blockpos2) && !worldgenlevel.getBlockState(blockpos2.below()).is(ModItems.SlimeKelp.get())) {

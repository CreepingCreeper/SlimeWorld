...
public class HugeSulfurSpikeFeature extends Feature<NoneFeatureConfiguration> {
    public HugeSulfurSpikeFeature(Codec<NoneFeatureConfiguration> p_66003_) {
        super(p_66003_);
    }
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> p_159882_) {
        ...
        if (!worldgenlevel.getBlockState(blockpos).is(ModTags.Blocks.SULFUR_FEATURE_REPLACEABLE)) {
            return false;
        } else {
                ...
                for(int i1 = -l; i1 <= l; ++i1) {
                    ...
                    for(int j1 = -l; j1 <= l; ++j1) {
                        ...
                        if ((i1 == 0 && j1 == 0 || !(f1 * f1 + f2 * f2 > f * f)) && (i1 != -l && i1 != l && j1 != -l && j1 != l || !(randomsource.nextFloat() > 0.75F))) {
                            ...
                            if (blockstate.isAir()) {
                                setBlock(worldgenlevel, blockpos.offset(i1, k, j1));
                            }
            ...
            for(int l1 = -k1; l1 <= k1; ++l1) {
                for(int i2 = -k1; i2 <= k1; ++i2) {
                    ...
                    while(blockpos1.getY() > 50) {
                        ...
                        if (!blockstate1.isAir() && !blockstate1.is(ModTags.Blocks.SULFUR_FEATURE_REPLACEABLE)) {
                            break;
                        }

                        setBlock(worldgenlevel, blockpos1);

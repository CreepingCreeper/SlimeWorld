...
@OnlyIn(Dist.CLIENT)
public class FloatingWindRenderer extends EntityRenderer<MagicbubbleEntity> {
    private static final ResourceLocation TEXTURE_LOCATION = SlimeWorld.getResource("textures/entity/magicbubble.png");
    ...
    private static void vertex(VertexConsumer consumer, Matrix4f pose, Matrix3f normal, int lightmapUV, float x, int y, int u, int v) {
        consumer.vertex(pose, x - 0.5F, (float)y - 0.25F, 0.0F).color(255, 255, 255, 255).uv((float)u, (float)v).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(lightmapUV).normal(normal, 0.0F, 1.0F, 0.0F).endVertex();
    }
